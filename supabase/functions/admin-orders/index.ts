...
serve(async (req) => {
  ...
  if (req.method === "GET") {
    ...
    const { data: orders, error: listError } = await supabase
      .from("orders")
      .select(
        "id, created_at, updated_at, amount, currency, status, fulfillment_status, fulfillment_updated_at, customer_name, customer_email, customer_phone, customer_address, customer_pin_code, razorpay_order_id, razorpay_payment_id, items, order_type, b2b_quote_id, b2b_account_id"
      )
